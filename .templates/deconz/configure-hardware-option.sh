...
# ----------------------------------------------------------------------
HARDWARE_OPTIONS="${TEMPLATE_DIR}/supported-hardware-options.txt"
INSTALLED_OPTION="${SERVICE_DIR}/installed-hardware-option.txt"


# ----------------------------------------------------------------------
# verify components in place
# ----------------------------------------------------------------------
if [ ! -r "${HARDWARE_OPTIONS}" ] ; then
	echo "Error: ${HARDWARE_OPTIONS} does not exist, or it exists but is not readable by $USER"
	exit 1
fi

if [ ! -w "${INSTALLED_OPTION}" ] ; then
	echo "Error: either ${INSTALLED_OPTION} does not exist, or it exists but is not writeable by ${USER}"
	exit 1
fi
...
SUPPORTED=$(sed -e '/^[#;]/d' "${HARDWARE_OPTIONS}" | sort)

# load list of add-on nodes that are currently installed
INSTALLED="$(sed -e '/^[#;]/d' "${INSTALLED_OPTION}")"
...
if [ $RC -eq 0 -a -n "${SELECTION}" ] ; then
	...
	# yes! initialise the new list of active nodes
	echo "# selection updated $(date)" >"${INSTALLED_OPTION}"

	# append selection
	echo "${SELECTION}" >>"${INSTALLED_OPTION}"
	
	# update ~/IOTstack/.env
	set_dot_env "DECONZ_DEVICE_PATH" "${SELECTION}" true
